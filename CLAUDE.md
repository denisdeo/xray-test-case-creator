...
- **API endpoints are required when the test involves an API call.** If a step sends or checks a request to an external API, include the endpoint in the `data` field so the tester knows exactly what is being called — e.g. `GET /trading/purchasing/offers?SiteIds=34490&Statuses=Finalised`. Keep the action itself plain English; the endpoint detail goes in `data`.
